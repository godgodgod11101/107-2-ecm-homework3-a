...
\begin{array}{lcl}
\mbox{Ans:}\\\\
\mbox{將}v_{it}=\epsilon_{it}-\alpha_{it}\mbox{代入}\frac{\sum_{it}(v_{it})(\alpha_{it})}{NT}\mbox{，得出}\frac{\sum_{it}(\epsilon_{it}-\alpha_{i})(\alpha_{it})}{NT}\mbox{，此為(1)式}\\
\mbox{因為}\alpha_{i}=\frac{\sum_{i=1}^T \epsilon_{it}}{T}=\bar{\epsilon_{i}}\mbox{代入(1)}\\
=\frac{\sum_{it}(\epsilon_{it}-\bar{epsilon_{i}})(\alpha_{it})}{NT}\\
\mbox{取期望值=}\frac{\sum_{it}\alpha_{it}[E(\epsilon_{it}-\bar{\epsilon}_i)]}{NT}=0\\
\mbox{故}v_{it}\mbox{和}\alpha_{it}\mbox{樣本covariance為零，有樣本上的零線性關聯}
\end{array}
$$
...
\begin{array}{lcl}
\mbox{Ans:}\\\\
\mbox{因為}\alpha_{i}=\frac{\sum_{i=1}^T\epsilon_{it}}{T}, v_{it}=\epsilon_{it}-\alpha_{i}\\
\mbox{故}E(v_{it}\alpha_{i})=E((\epsilon_{it}-\frac{\sum_{i=1}^T\epsilon_{it}}{T})*(\frac{\sum_{i=1}^T\epsilon_{it}}{T})\\
...
(2)\\
\mbox{因為}\alpha_i=\frac{\sum_{i=1}^T\epsilon_{it}}{T}, v_{it}=\epsilon_{it}-\alpha_{i}\\
\mbox{可得}\epsilon_{it}=v_{it}+\alpha_{i}\\
\mbox{求}v(\epsilon_{it})=v(v_{it}+\alpha_{i})\\
=v(v_{it}+\alpha_{i})\\
...
Ans:去除$\alpha$影響的非解釋變數x 造成的y的因素
